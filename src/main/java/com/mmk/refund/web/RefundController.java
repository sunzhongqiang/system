...
import com.mmk.refund.constants.RefundConfig;
...
public class RefundController extends BaseController {
	...
	public ResultMsg refundAgree(Long id) {
		...
		if ("0".equals(refund.getHasGoodsReturn())) {
			if ("AUTO".equals(RefundConfig.AUTO_REFUND)) {
	 ...
	 *            退款退货ID
	 */
	@RequestMapping("/refund/confirmGoods")
	public ResultMsg confirmGoods(Long id) {
		log.info("确认收货");
		Refund refund = refundService.find(id);
		if ("AUTO".equals(RefundConfig.AUTO_REFUND)) {
			if (refundService.refundMoney(refund)) {
		...
		refundService.save(refund);
		return new ResultMsg(true, "确认收货");
	}
