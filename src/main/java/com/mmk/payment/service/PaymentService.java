package com.mmk.payment.service;

import java.util.List;
import com.mmk.gene.service.BaseService;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import com.mmk.payment.model.Payment;
import com.mmk.payment.condition.PaymentCondition;

/**
* PaymentService: 支付方式 业务服务层接口
*2016-12-05 11:56:57
*@author 孙中强 sunzhongqiang
*@version 1.0
*/
public interface PaymentService extends BaseService<Payment, Long> {
    /**
     * 生成的列表分页查询方法
     * @param paymentCondition  查询条件
     * @param pageable 分页参数
     * @return 分页返回查询的结果
     * @author 孙中强 sunzhongqiang
     * 
     */
    Page<Payment> list(PaymentCondition paymentCondition, Pageable pageable);
    
    /**
     * 不分页查询相关信息，根据传入的model类对象取得查询结果集List
     * @param  payment 查询类
     * @return 查询的结果集
     * @author 孙中强 sunzhongqiang
     * 
     */
    public List<Payment> list(PaymentCondition payment);

    /**
     * 根据给定的字段和属性值，获得符合条件的第一个结果
     * @param field Payment 中的某个字段
     * @param value 字段的值
     * @return Payment 返回符合条件的结果，如果没有返回null
     * @author 孙中强 sunzhongqiang
     * 
     * 
     */
    Payment findBy(String field,Object value);
    
    /**
     * 根据给定的字段和属性值，获得符合条件的所有结果
     * @param field Payment中的某个字段
     * @param value 字段的值
     * @return 返回符合条件的所有结果
     * @author 孙中强 sunzhongqiang
     * 
     * 
     */
    List<Payment> findAllBy(String field,Object value);
}